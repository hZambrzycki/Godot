...
@export var magnet_strenght : float = 1.0
@export var player_magnet_audio : bool = false
...
func _process(delta):
	for i in range(items.size()-1, -1, -1):
		...
		if _item == null:
			...
			speeds[i] += magnet_strenght * delta 
...
func _on_area_enter(_a : Area2D) -> void:
	if _a.get_parent() is ItemPickup:
		...
		speeds.append(magnet_strenght)
		_new_item.set_physics_process(false)
		if player_magnet_audio:
			audio.play(0)
